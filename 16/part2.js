...
function dfs2(cur, rest, t) {
  let max = dfs(AA, rest, 26);
  for (const [r, rr] of chooseOne(rest)) {
    if (dist[cur][r] < t) {
      const f = flows[r] * (t - dist[cur][r] - 1) + dfs2(r, rr, t - dist[cur][r] - 1);
      if (f > max) {
        max = f;
      }
    }
...
const r = dfs2(AA, flows.map((x, i) => [x,i]).filter(f => f[0] > 0).map(x => x[1]), 26);
console.log(r);
